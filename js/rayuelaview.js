...
const RAY_TXT = {
  ...
  recorrido: "Zure ibilbidea", nuevo: "Amaiera berria!", todos: "Sareko hamaika amaierak aurkitu dituzu!",
...
function rayMapa(){
  const R = RAYUELA_HF, U = 56, P = 34, W = 11.6 * U + 2 * P, H = 15.2 * U + P;
  ...
  for (const [n, s] of Object.entries(R.estaciones)) for (const o of s.opciones){
    const k = n + ">" + o.to; if (seen.has(k)) continue; seen.add(k);
    const cls = now.has(k) ? "ray-e now" : hist.has(k) ? "ray-e hist" : "ray-e";
    edges += '<path class="' + cls + '" d="' + rayPath(s.xy, rayXY(o.to), U) + '" stroke="' + rayLinea(s.linea) + '"/>';
  }
  let nodes = "";
  for (const [n, s] of Object.entries(R.estaciones)){
    const [x, y] = s.xy.map(v => v * U), c = rayLinea(s.linea), v = vis.has(n);
    const cls = "ray-st" + (v ? " vis" : "") + (n === actual ? " cur" : "") + (s.tipo === "contradiccion" ? " contra" : "") + (s.tipo === "vida" ? " vida" : "");
    const label = (v || nombres) ? '<text class="ray-lbl" x="' + (x + (s.xy[0] > 9.4 ? -24 : 24)) + '" y="' + (y + 4) + '" text-anchor="' + (s.xy[0] > 9.4 ? "end" : "start") + '">' + rayEsc(s.titulo.replace(/^[^:]*:s*/, "")) + '</text>' : "";
  ...
  for (const [k, t] of Object.entries(R.terminales)){
...
function rayStats(){
  const d = rayData();
  return '<p class="ray-stats">' + RAY_TXT.viajes + ' <b>' + (d.viajes || []).length + '</b> · ' + RAY_TXT.finales + ' <b>' + rayFinales().size + ' / ' + Object.keys(RAYUELA_HF.terminales).length + '</b> · ' +
    RAY_TXT.estaciones + ' <b>' + [...rayVisitadas()].filter(n => RAYUELA_HF.estaciones[n]).length + ' / ' + Object.keys(RAYUELA_HF.estaciones).length + '</b></p>';
}
...
function renderRayEstacion(){
  ...
  rayLayout('<button class="linkbtn ray-back" id="rayquit">' + RAY_TXT.abandonar + '</button>' +
    '<article class="ray-card ' + s.tipo + '" style="--lc:' + rayLinea(s.linea) + '">' +
      ...
      '<div class="ray-opts">' + s.opciones.map((o, i) => '<button class="ray-opt" data-i="' + i + '"><span class="ray-n">' + (i + 1) + '</span><span>' + rayEsc(o.t) + '</span></button>').join("") + '</div>' +
    '</article>');
  ...
  rayBox().querySelectorAll("[data-i]").forEach(b => b.addEventListener("click", () => {
    const o = s.opciones[+b.dataset.i];
...
function renderRayPlano(){
  ...
  const term = Object.entries(RAYUELA_HF.terminales).map(([k, t]) => '<div class="ray-tcard' + (fins.has(k) ? " got" : "") + '"><span class="ray-badge term">' + k + '</span>' +
